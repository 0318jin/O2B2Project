		...
		try { 
			...
			while (true) {
				...
				if (readData.equals("1")) { // 로그인 버튼"1"
					...
					System.out.println("친구 추가 조건문");
					And_addfr maddfr = And_addfr.getInstance();
					maddfr.addfr_start();
				}else if(readData.equals("7")) {
					System.out.println("서버 연결이 끊겼습니다.");
					And_SocketThread mAnd = And_SocketThread.get();
					InetAddress mI = mAnd.sock.getInetAddress();
					JOptionPane.showMessageDialog(null,  "안드로이드 App Ip : " + mI.getHostAddress() + " 접속이 끊겼습니다.");
